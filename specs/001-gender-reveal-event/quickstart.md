...
10. **디자인/타이포그래피 확인** (`public/reference/step1~4.png`와 육안 비교, FR-016~FR-020):
    - 브랜드 타이틀·각 화면 헤드라인이 픽셀 폰트(RoundedFixedsys)로, 라벨·입력값·본문이
      Pretendard로 렌더링되는지 확인(FR-016).
    - 전체 배경이 화이트이고 장식 요소가 픽셀 아트 톤앤톤으로 통일되어 있는지 확인(FR-017).
    - Step 1에서 '아들' 토글은 파스텔 블루, '딸' 토글은 파스텔 핑크 배경인지 확인(FR-018).
    - Step 3 결과 화면에서 성별에 맞는 강조 문구·예정일 색상(아들=블루, 딸=핑크)과 픽셀 아트
      캐릭터 일러스트가 노출되는지 확인(FR-019).
    - Step 2에서 풍선 주변에 핑크/블루 하트 파티클이 교차 배치되어 있고, "N / 10" 진행
      카운터가 표시되는지 확인(FR-020).
